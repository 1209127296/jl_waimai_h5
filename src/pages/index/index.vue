...
<script>
  ...
  export default {
          ...
          if (res.data.status == 1) {
            ...
            this.fly.post("address/findAddress").then(res=>{
              if(res.data.obj.length>0){
                return
              }else if(res.data.obj.length==0){
                this.newDefaultAddress = true
              }
            })
          } else {
            console.log("登录失败");
    ...
    onShow(){
      ...
      try{
        console.log(this.$root.$mp.query.newDefaultAddress)
        if(this.$root.$mp.query.newDefaultAddress=="false"||this.$root.$mp.query.newDefaultAddress==undefined){
          this.newDefaultAddress=false
        }
      }catch(error){
        console.log(error)
        return
