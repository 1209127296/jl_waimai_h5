...
export default {
  ...
  methods:{
    ...
    saveAddress(){
      ...
      if(this.mobile.length<11||this.location==""||this.address_detail==""||this.name==""){
        wx.showToast({
          title:"请检查收货人信息填写是否正确",
          icon:"none",
          duration:2000
        })
      }else{
        if(this.isedit==false){
          this.fly.post("address/addAddress",{
            ...
            if(pageNum.length==1){
              wx.reLaunch({
                url:"/pages/index/main?newDefaultAddress="+false
              })
            }else{
              wx.navigateBack({
                delta: -1
          ...
          this.fly.post("address/updateAddress",{
            ...
            if(res.data.code>0){
              ...
              wx.showToast({
                title:"修改地址失败，必须要有一条默认地址或者检查地址是否输入正确",
                icon:"none",
                duration:2000
              })
            }
